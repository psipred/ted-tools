...
JOB_COMPLETE_FILE=${SGE_O_WORKDIR}/job_complete.txt
...
log "JOB_COMPLETE_FILE   : $JOB_COMPLETE_FILE"
...
process_zipfile () {
    ...
    then
        log "Checking status of '$zipfile' in job complete file ..."
        grep "$zipfile" $JOB_COMPLETE_FILE | grep OK && zip_status=$? || zip_status=$?
        log "STATUS: $zipfile $zip_status"
        if [ "$zip_status" == "0" ]
        then
            log "Found status OK, skipping..."
            return
        else
            log "Failed to find status OK, running..."
        fi
    fi

...
def generate_features_domain(file: str, device: torch.device, zipped=False):
    ...
    pdb, ca, backbone, _ = pdb_to_features(file, zipped=zipped)

    aa, encoded = encode_seq(ca["resn"], three_letter=True)
    seq = torch.tensor(encoded).long()
    
    md5 = hashlib.md5(''.join(aa).encode('utf-8')).hexdigest()
    ...
    return s, z, r, t, ri, pdb, b, md5

def pdb_to_features(file, resi=None, zipped=False):

    pdb, remarks = open_pdb(file, zipped=zipped)
    ...
    return pdb[0], ca, backbone, remarks
...
def encode_seq(seq, three_letter=False):
    ...
    return seq, (np.frombuffer(''.join(seq).translate(aa_trans).encode(

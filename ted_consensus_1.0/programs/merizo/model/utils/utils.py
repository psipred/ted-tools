...
def assimilate_short_terminal(dom_ids, dom_counts, threshold, termini):
    ...
    short_seg = dom_counts[:threshold] < threshold
    if False in short_seg:
        dom_ids[:threshold] = dom_ids[threshold]
    ...
    if save_domains:
        for i, u in enumerate(dom_ids[dom_ids.nonzero()].unique()):
            ...
            if output:
                ...
                write_pdb(p, outname + "_" + str(i+1).zfill(2) + ".pdb", comment)

    write_pdb(pdb, outname + ".pdb", comment)

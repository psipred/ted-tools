...
def open_pdb(file: str, ignore_H: bool = False, zipped = False):
    ...
    molecules, mol, remarks = [], [], []
    
    if zipped is not False:
        zf = zipped.read(file)
        pdbfile = zf.decode("utf-8").split('\n')
    else:
        pdbfile = open(file, 'r').readlines()
